...
def display_dashboard(env, step_num, cum_reward, last_reward=None, last_action_desc=None):
    ...
    print("=" * 80)
    print(f"{C_HEADER}                 SPACECRAFT CONTROL SYSTEM DASHBOARD{C_RESET}")
    print("=" * 80)
    ...
    print(f"  Simulation Time:   {sim_time:.1f}s / {time_limit:.1f}s")
    print(f"  Mission Time Bar:  {make_progress_bar(time_frac, length=30, fill_char='▰', empty_char='▱')}")
    print(f"  Orbits Completed:  {orbit_frac:.2f} orbits")
    ...
    try:
        ...
        # Convert bits to Megabits (Mb) or Megabytes (MB)
        # 1 Mb = 1e6 bits
        ...
        # So we construct progress bar manually or invert colors
    ...
    while True:
        ...
        try:
            while True:
                ...
                # Step the simulation
                print(f"\nStepping simulation with action {action_idx}...")
                ...
                if terminated or truncated:
                    ...
                    if terminated:
                        ...
                        if hasattr(dyn, "altitude_valid") and not dyn.altitude_valid():
                            reasons.append("Altitude invalid (spacecraft has deorbited)")
